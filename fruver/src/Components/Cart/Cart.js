...
import StylesCart from "./StylesCart.css"
...
const Cart = () => {
  ...
  return (
    <div className={StylesCart.cartContainer}>
      <div onClick={() => setCartOpen(!cartOpen)}className={StylesCart.buttonCartContainer}>
        <div className={StylesCart.buttonCart}>
          {!cartOpen ? (
            <svg
              className={StylesCart.open}
              width={"35px"}
              viewBox="0 0 30 27"
              fill="#F0F0F0"
        ...
        {!cartOpen && (
          <div className={StylesCart.productsNumber}>{productsLength}</div>
        )}
      </div>
